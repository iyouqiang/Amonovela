...
open class KLinePainterView: UIView {
    ...
    var fuzzylayer: CALayer = {
        ...
        layer.backgroundColor = COLOR_KLineColorAlpha(0x000000, alpha: 0.3).cgColor
        layer.cornerRadius = 10
        layer.masksToBounds = true
        return layer
    }()
    
    public init(frame: CGRect,datas:  [KLineModel], scrollX: CGFloat,isLine: Bool, scaleX: CGFloat, isLongPress: Bool, mainState: MainState, secondaryState: SecondaryState) {
    ...
    func drawRealTimePrice(context: CGContext) {
        ...
        var textColor = ChartColors.reightTextColor
        if lastModel?.close ?? 0.0 > point.open {
            textColor = ChartColors.dnColor
        }else {
