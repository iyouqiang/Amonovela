...
class DataUtil {
    ...
    static func addLastData(dataList: [KLineModel], data: KLineModel) {
        ...
        /// 指标计算
        calculateIndicators(dataList: _dataList)
     }
    
    static func calculateIndicators(dataList: [KLineModel]) {
        
        calcMA(dataList, isLast: true);
        calcBOLL(dataList, isLast: true);
        calcVolumeMA(dataList, isLast: true);
        calcKDJ(dataList, isLast: true);
        calcMACD(dataList, isLast: true);
        calcRSI(dataList, isLast: true);
        calcWR(dataList, isLast: true);
    }
